...
const FacilityAction = ({ data, token, fetchData }) => {
    ...
    const toggleStatusChange = async (status, onClose) => {
        ...
        try {
            ...
            const response = await adminService.toggleFacilites(params, token)
    ...
    return (
        ...
        >
            {({ onClose }) => (
                <div className="py-1">
                    <Link href={`facilities/${data.id}`} passHref>
                    ...
                    <button disabled={loading}>
                        ...
                        >
                            ...
                            <span>{data?.status === 'PENDING' || data?.status === 'SUSPENDED' ? 'Activate Facility' : 'Suspend Facility'}</span>
                        </div>
                    </button>
                </div>
            )}
